<script lang="ts">
    ...
    import { ApiError, NetworkError, getGamemodeStats, getGlobalMetrics, getLanguageStats } from '$lib/api';
    import type { GamemodeStats, LanguageStats, GlobalMetric } from '$lib/types/stats';
    import { parseDatetime, formatMetricTime, type MetricTimeRange } from '$lib/format/datetime';
    ...
    onMount(async () => {
        try {
            const [lang, gm] = await Promise.all([getLanguageStats(), getGamemodeStats()]);
            languageStats = lang;
            gamemodeStats = gm;
        } catch (e) {
            loadError =
...
<svelte:head>
    ...
    <meta name="description" content="Statistics about SA-MP and open.mp servers" />
</svelte:head>

<div>
    <h2>Statistics</h2>
    <p>
        SAMonitor accounts for the total amount of servers and players a few times every hour,
        of every day.
    </p>
    <div>
        <div class="innerContent">
            ...
            <div style="margin-top: 1rem">
                <input
                    type="button"
                    value={showTable ? 'Hide weekly stats table.' : "Show last week's stats in a table."}
                    onclick={toggleTable}
                />
            </div>
            {#if showTable}
                ...
                {:else if tableMetrics}
                    <table style="width: 100%; border: 1px rgb(128, 128, 128) solid; margin-top: 1rem;">
                        <thead>
                            <tr>
                                <th>Time</th>
                        ...
                        <tbody>
                            {#each tableMetrics as instant (instant.time)}
                                <tr>
                                    ...
                                    <td>{instant.players.toLocaleString('en-US')}</td>
                                    <td>{instant.servers.toLocaleString('en-US')}</td>
                                </tr>
                            {/each}
                        </tbody>
                    </table>
