<script lang="ts">
    ...
    const languageRows = $derived(
        (Object.entries(languageStats) as [keyof LanguageStats, { amount: number; players: number }][])
    ...
    const gamemodeRows = $derived(
        (Object.entries(gamemodeStats) as [keyof GamemodeStats, { amount: number; players: number }][])
            .filter(([key]) => key !== 'other')
            .map<Row>(([key, v]) => ({
...
<div class="innerContent">
        ...
        <tbody>
            {#each languageRows as row (row.name)}
                <tr>
                    <td>{row.name}</td>
                    <td>{row.servers}</td>
                    <td>{formatThousands(row.players)}</td>
                </tr>
            {/each}
        </tbody>
    </table>
    <p><small>The other {otherLanguageAmount} servers don't have a defined language.</small></p>
    ...
    <p><small>The other {otherGamemodeAmount} servers don't have a defined gamemode.</small></p>

    <p style="margin-top: 1rem">
        There seems to be a practice to use the 'Language' or 'Gamemode' fields for the
        name of the server rather than what they actually are. Server owners, please, this makes
        it harder for people to find your server.
    </p>
</div>
